...
using static ProjetoRecepcao.Identidade.PlanilhaReposicao;
...
namespace ProjetoRecepcao.Controllers
{
  ...
  public class PlanilhaReposicaoController : Controller
  {
    ...
    [HttpPost("/salvar-planilhas")]
    public async Task<ActionResult<string>> CreatePlanilhasGrande([FromBody] JsonElement dados)
    {
      if (dados.ValueKind == JsonValueKind.Null)
      {
      ...
      try
      {
        // Acessando diretamente as propriedades com GetProperty
        if (dados.TryGetProperty("dias", out JsonElement dias))
        {
          // Processa os dados de dias
        }
        else
        {
          return BadRequest("Dados de dias não encontrados.");
        }

        if (dados.TryGetProperty("tabelas", out JsonElement tabelas))
        {
          // Processa as tabelas
        }
        else
        {
          return BadRequest("Dados de tabelas não encontrados.");
        }

        // Processa os dados para gerar a lista de PlanilhaReposicao
        var planilhas = new List<PlanilhaReposicao>();

        foreach (var horario in tabelas.EnumerateObject())
        {
          foreach (var dia in dias.GetProperty("days").EnumerateArray())
          {
            if (dia.GetProperty("isSelected").GetBoolean())
            {
              var alunos = horario.Value.GetProperty(ConvertDiaParaString(dia.GetProperty("weekday").GetString()));
              foreach (var aluno in alunos.EnumerateArray())
              {
                var planilha = new PlanilhaReposicao(
                    Guid.NewGuid(),
                    aluno.GetProperty("nome").GetString(),
                    aluno.GetProperty("horario").GetString(),
                    aluno.GetProperty("data").GetString(),
                    "Professor",
                    dia.GetProperty("weekday").GetString()
                );
        ...
        await _reposicaoService.CreatePlanilhaReposicao(planilhas);

        return Ok("Horários salvos com sucesso");
      }
      catch (Exception ex)
      {
    ...
    // Método para converter o nome do dia para a chave do horário (segunda, terca, etc.)
    private string ConvertDiaParaString(string nomeDia)
    {
      switch (nomeDia)
      {
